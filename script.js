...
function revealHeroSequence() {
  // 1. Kalimat "Happy Birthday, Sayang!" muncul huruf demi huruf
...
const WHATSAPP_NUMBER = '6283160127247'; // Nomor WA kamu

sendWishBtn.addEventListener('click', () => {
  const text = wishInput.value.trim();
  const finalMessage = text
    ? text
    : 'Halo! Aku baru buka website ulang tahun buat Maorent, seru banget 🎉';
  
  // Gunakan whatsapp:// agar langsung memicu app WA (HP/Desktop)
  const appUrl = 'whatsapp://send?phone=' + 6283160127247 + '&text=' + encodeURIComponent(finalMessage);
  
  // Fallback link web jika di laptop/browser belum terinstall WA Desktop
  const webUrl = 'https://api.whatsapp.com/send?6283160127247=' + 6283160127247 + '&text=' + encodeURIComponent(finalMessage);
